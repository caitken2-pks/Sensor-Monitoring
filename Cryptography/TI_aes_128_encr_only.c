/*
 * TI_aes_128_encr_only.c - AES-128 software encryption (transmitter nodes)
 *
 * Optimised encryption-only variant of the TI AES-128 library.
 * Omits decryption tables (rsbox) and all decryption code to
 * reduce flash footprint for transmitter-only sensor nodes.
 *
 * FIPS-197 compliant.
 */

#include "TI_aes_128_encr_only.h"

/* ---------------------------------------------------------------------------
 * AES S-box (encryption substitution table, FIPS-197 Figure 7)
 * ---------------------------------------------------------------------------*/
static const unsigned char sbox[256] = {
    
};

/* Round constants for key schedule */
static const unsigned char rcon[11] = {
    0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

/* Number of AES-128 rounds */
#define AES_NR  10

/* ---------------------------------------------------------------------------
 * GF(2^8) multiply-by-2
 * ---------------------------------------------------------------------------*/
static unsigned char xtime(unsigned char x)
{
    return (unsigned char)(((x << 1) ^ ((x & 0x80) ? 0x1b : 0x00)) & 0xff);
}

/* ---------------------------------------------------------------------------
 * Key expansion
 * ---------------------------------------------------------------------------*/
static void key_expansion(const unsigned char *key,
                           unsigned char       *roundKey)
{
    unsigned char temp[4];
    unsigned char k;
    int i;

    for (i = 0; i < AES_KEY_SIZE; i++) {
        roundKey[i] = key[i];
    }

    for (i = AES_KEY_SIZE; i < (AES_NR + 1) * AES_BLOCK_SIZE; i += 4) {
        temp[0] = roundKey[i - 4];
        temp[1] = roundKey[i - 3];
        temp[2] = roundKey[i - 2];
        temp[3] = roundKey[i - 1];

        if ((i / 4) % (AES_KEY_SIZE / 4) == 0) {
            k = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = k;

            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];

            temp[0] ^= rcon[i / AES_KEY_SIZE];
        }

        roundKey[i + 0] = roundKey[i - AES_KEY_SIZE + 0] ^ temp[0];
        roundKey[i + 1] = roundKey[i - AES_KEY_SIZE + 1] ^ temp[1];
        roundKey[i + 2] = roundKey[i - AES_KEY_SIZE + 2] ^ temp[2];
        roundKey[i + 3] = roundKey[i - AES_KEY_SIZE + 3] ^ temp[3];
    }
}

/* ---------------------------------------------------------------------------
 * AddRoundKey
 * ---------------------------------------------------------------------------*/
static void add_round_key(unsigned char *state, const unsigned char *roundKey,
                           int round)
{
    int i;
    for (i = 0; i < AES_BLOCK_SIZE; i++) {
        state[i] ^= roundKey[round * AES_BLOCK_SIZE + i];
    }
}

/* ---------------------------------------------------------------------------
 * SubBytes
 * ---------------------------------------------------------------------------*/
static void sub_bytes(unsigned char *state)
{
    int i;
    for (i = 0; i < AES_BLOCK_SIZE; i++) {
        state[i] = sbox[state[i]];
    }
}

/* ---------------------------------------------------------------------------
 * ShiftRows
 * ---------------------------------------------------------------------------*/
static void shift_rows(unsigned char *state)
{
    unsigned char tmp;

    tmp       = state[1];
    state[1]  = state[5];
    state[5]  = state[9];
    state[9]  = state[13];
    state[13] = tmp;

    tmp       = state[2];
    state[2]  = state[10];
    state[10] = tmp;
    tmp       = state[6];
    state[6]  = state[14];
    state[14] = tmp;

    tmp       = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7]  = state[3];
    state[3]  = tmp;
}

/* ---------------------------------------------------------------------------
 * MixColumns
 * ---------------------------------------------------------------------------*/
static void mix_columns(unsigned char *state)
{
    int col;
    unsigned char s0, s1, s2, s3;
    unsigned char x2s0, x2s1, x2s2, x2s3;

    for (col = 0; col < 4; col++) {
        s0   = state[col * 4 + 0];
        s1   = state[col * 4 + 1];
        s2   = state[col * 4 + 2];
        s3   = state[col * 4 + 3];
        x2s0 = xtime(s0);
        x2s1 = xtime(s1);
        x2s2 = xtime(s2);
        x2s3 = xtime(s3);

        state[col * 4 + 0] = (unsigned char)(x2s0 ^ (x2s1 ^ s1) ^ s2 ^ s3);
        state[col * 4 + 1] = (unsigned char)(s0 ^ x2s1 ^ (x2s2 ^ s2) ^ s3);
        state[col * 4 + 2] = (unsigned char)(s0 ^ s1 ^ x2s2 ^ (x2s3 ^ s3));
        state[col * 4 + 3] = (unsigned char)((x2s0 ^ s0) ^ s1 ^ s2 ^ x2s3);
    }
}

/* ---------------------------------------------------------------------------
 * aes_encrypt – public entry point
 * ---------------------------------------------------------------------------*/
void aes_encrypt(unsigned char *state, unsigned char *key)
{
    unsigned char roundKey[(AES_NR + 1) * AES_BLOCK_SIZE];
    int round;

    key_expansion(key, roundKey);

    add_round_key(state, roundKey, 0);

    for (round = 1; round < AES_NR; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, roundKey, round);
    }

    /* Final round – no MixColumns */
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, roundKey, AES_NR);
}
